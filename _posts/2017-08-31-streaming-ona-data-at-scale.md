...
> Alternatively, if you want to put some data in Druid and skip the NiFi and Kafka steps, you can load a JSON file on disk directly into Druid. To do this here is an example JSON configuration:
```json
{
  "type" : "index_hadoop",
  "spec" : {
...
```
> This assumes you have valid JSON data from the Ona API in the file `/path/ona-api-demo.json` and that the `_submission_time` of that data is between the value in `intervals`. Go ahead and adjust those based on your demo data.
> Assuming you have saved an adjusted file like the above to `ona-api-index.json`, you can load a demo JSON file into Druid using:
```
...
<img alt="superset-cluster" src="/assets/images/2017-08-31/superset-cluster.png" width="600px"/>

Now go back to the `Sources` menu at the top and select `Refresh Druid Metadata`. After this, again in the `Sources` menu navigate to `Druid Datasources` and you will see your `ona-json` Datasource. This is the datasource either streaming in, if you used NiFi and Kafka setup, or from the static JSON file, if you posted data to the indexer.
...
We will still use one-time pulls from the API to synchronize historical data from Ona into HDFS and then into Druid.
...
One potential solution is to build semi-relational structure by adding a UUID to the incoming JSON and then inserting that as a foreign `parent` key in each nested JSON record with a `label` key whose value is the key of the nested JSON in the original record. For example, assuming we reserve the `__.+__` namespace for our internal keys:
...
Would become the three records. The first of which is the outer record and has an assigned `__uuid__`:
...
The major weakness of this approach is that it adds a significant amount of structural complexity to the datasets. Relatedly, if we expected all our records to have the same key value pairs, they no longer would. An alternative approach is to normalize?????? our data by pushing all nested keys to the top layer and create keys using the parent keys as prefix. Again reserving the `__.+__` namespace for generated keys and as our own separator, and using the same example dataset, this would produce a final JSON map of:
